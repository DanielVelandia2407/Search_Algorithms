...
public class SecondaryIndexView extends JFrame {
    ...
    public void mostrarEstructura(int bloques, int bfri, int totalDatos) {
        panelEstructura.removeAll();

        // Panel superior: Índice
        JPanel panelIndice = new JPanel(new FlowLayout());
        panelIndice.setBackground(new Color(255, 250, 240));
        panelIndice.setBorder(BorderFactory.createTitledBorder("Bloques de Índice (" + bloques + ")"));

        for (int i = 1; i <= Math.min(bloques, 5); i++) {
            int filas = Math.min(bfri, 5);
            JPanel bloque = new JPanel(new GridLayout(filas, 1));
            bloque.setPreferredSize(new Dimension(80, 100));
            bloque.setBorder(BorderFactory.createLineBorder(Color.GRAY));

            for (int j = 1; j <= filas; j++) {
                String texto = (j == filas) ? "R" + bfri : "R" + j;
        ...
        // Panel inferior: Fichero de datos
        JPanel panelFichero = new JPanel(new FlowLayout());
        panelFichero.setBackground(new Color(240, 255, 240));
        panelFichero.setBorder(BorderFactory.createTitledBorder("Registros de Datos"));
        ...
        for (int i = 1; i <= Math.min(bloques, 5); i++) {
            ...
            for (int j = 1; j <= filas; j++) {
                String texto = (j == filas) ? "D" + totalDatos : "D" + j;
                JLabel celda = new JLabel(texto, SwingConstants.CENTER);
                celda.setOpaque(true);
                celda.setBackground(new Color(152, 251, 152));
                celda.setForeground(Color.BLACK);
            ...
            panelFichero.add(bloque);
        }

        panelEstructura.add(panelIndice);
        panelEstructura.add(panelFichero);
        panelEstructura.revalidate();
        panelEstructura.repaint();
    }
