...
import view.ExternalModSearchView;
...
public class ExternalModSearchController {

    private final ExternalModSearchView view;
    ...
    public ExternalModSearchController(ExternalModSearchView view) {
        this.view = view;
    ...
    private void loadDataFromFile() {
        ...
        try {
            File file = new File("src/utilities/datos-hash-modulo-externo.txt");
    ...
    private void initializeEmptyHashTable() {
        ...
        System.out.println("Tabla hash externa inicializada con " + tableSize + " bloques");
    }
    ...
    private void displayDataInTable() {
        ...
        view.setHashFunction("value % " + tableSize);
    }
    ...
    private void performNormalSearch() {
        ...
        try {
            ...
            int hashPosition = valueToSearch % tableSize;
            ...
            if (result.found) {
                ...
                view.setResultMessage("Clave " + valueToSearch + " no encontrada. " +
                        "Función hash: " + valueToSearch + " % " + tableSize + " = " + (hashPosition + 1) +
    ...
    private void performAnimatedSearch() {
        ...
        try {
            ...
            blockAccessCount = 0;
            view.setBlockAccessCount(blockAccessCount);
            view.setCurrentOperation("Búsqueda hash externa");
            view.clearHighlights();
            ...
            SwingWorker<SearchResult, SearchProgress> worker = new SwingWorker<SearchResult, SearchProgress>() {
                ...
                protected SearchResult doInBackground() throws Exception {
                    ...
                    blockAccessCount++;
                    publish(new SearchProgress(hashPosition, blockAccessCount, "Accediendo bloque hash"));
                    Thread.sleep(600);
                    ...
                    while (currentPosition != hashPosition && attempts < tableSize - 1) {
                        blockAccessCount++;
                        publish(new SearchProgress(currentPosition, blockAccessCount, "Resolviendo colisión"));
                        Thread.sleep(500);
    ...
    private SearchResult externalHashSearch(int target) {
        int hashPosition = target % tableSize;
        ...
        // If not found in hash position, try collision resolution
        int currentPosition = (hashPosition + 1) % tableSize;
        int attempts = 0;
    ...
    private void generateNewHashTable(int newTableSize) {
        ...
        view.setResultMessage("Tabla hash externa generada. Rango permitido: " + minValue + " - " + maxValue, true);
        ...
        if (found) {
            ...
            if (foundBlock == hashPosition) {
                view.setResultMessage("Clave " + value + " eliminada del Bloque " + (foundBlock + 1) +
                        ", Slot " + (foundSlot + 1) + " (posición hash original)", true);
            } else {
                view.setResultMessage("Clave " + value + " eliminada del Bloque " + (foundBlock + 1) +
                        ", Slot " + (foundSlot + 1) +
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ExternalModSearchView newView = new ExternalModSearchView();
            ExternalModSearchController controller = new ExternalModSearchController(newView);
            newView.showWindow();
        });
    }
}