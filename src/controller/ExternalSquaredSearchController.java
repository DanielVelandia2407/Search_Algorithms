...
import view.ExternalSquaredSearchView;
...
public class ExternalSquaredSearchController {

    private final ExternalSquaredSearchView view;
    ...
    public ExternalSquaredSearchController(ExternalSquaredSearchView view) {
        this.view = view;
    ...
    private void loadDataFromFile() {
        ...
        try {
            File file = new File("src/utilities/datos-hash-cuadrado-externo.txt");
    ...
    private void initializeEmptyHashTable() {
        ...
        System.out.println("Tabla hash externa cuadrado inicializada con " + tableSize + " bloques");
    }
    ...
    private void displayDataInTable() {
        ...
        view.setHashFunction("cuadrado medio % " + tableSize);
    }

    // Method to calculate middle square hash for external storage
    ...
    private void performNormalSearch() {
        ...
        try {
            ...
            int hashPosition = calculateMiddleSquareHash(valueToSearch);
            ...
            if (result.found) {
                ...
                view.setResultMessage("Clave " + valueToSearch + " encontrada en Bloque " +
                        ...
                        (result.blockIndex == hashPosition ? " (posición hash original cuadrado medio)" :
                                " (reubicada por colisión, hash original: " + (hashPosition + 1) + ")"), true);
            } else {
                view.setResultMessage("Clave " + valueToSearch + " no encontrada. " +
                        "Función hash cuadrado medio: " + valueToSearch + "² = " +
                        ((long)valueToSearch * valueToSearch) + " → " + (hashPosition + 1) +
    ...
    private void performAnimatedSearch() {
        ...
        try {
            ...
            blockAccessCount = 0;
            view.setBlockAccessCount(blockAccessCount);
            view.setCurrentOperation("Búsqueda hash cuadrado externa");
            view.clearHighlights();
            ...
            SwingWorker<SearchResult, SearchProgress> worker = new SwingWorker<SearchResult, SearchProgress>() {
                ...
                protected SearchResult doInBackground() throws Exception {
                    ...
                    blockAccessCount++;
                    publish(new SearchProgress(hashPosition, blockAccessCount, "Accediendo bloque hash cuadrado"));
                    Thread.sleep(600);
    ...
    private SearchResult externalHashSearch(int target) {
        int hashPosition = calculateMiddleSquareHash(target);
    ...
    private void generateNewHashTable(int newTableSize) {
        ...
        view.setResultMessage("Tabla hash cuadrado externa generada. Rango permitido: " + minValue + " - " + maxValue, true);
        ...
        for (int slotIndex = 0; slotIndex < block.size(); slotIndex++) {
            if (block.get(slotIndex) == -1) {
                ...
                view.setResultMessage("Clave " + value + " insertada en Bloque " + (hashPosition + 1) +
                        ", Slot " + (slotIndex + 1) + " (hash cuadrado medio: " + value + "² = " +
                        ((long)value * value) + " → " + (hashPosition + 1) + ")", true);
                return;
            }
        }
        ...
        if (found) {
            ...
            if (foundBlock == hashPosition) {
                view.setResultMessage("Clave " + value + " eliminada del Bloque " + (foundBlock + 1) +
                        ", Slot " + (foundSlot + 1) + " (posición hash cuadrado medio original)", true);
            } else {
                view.setResultMessage("Clave " + value + " eliminada del Bloque " + (foundBlock + 1) +
                        ", Slot " + (foundSlot + 1) +
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ExternalSquaredSearchView newView = new ExternalSquaredSearchView();
            ExternalSquaredSearchController controller = new ExternalSquaredSearchController(newView);
            newView.showWindow();
        });
    }
}
